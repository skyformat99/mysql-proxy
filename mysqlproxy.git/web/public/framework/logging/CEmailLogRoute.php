...
class CEmailLogRoute extends CLogRoute
{
	...
	protected function processLogs($logs)
    {
        try {
            ...
            if ($env == 'TEST') {
                $this->setSubject('测试环境');
                $isSendSms = false;
            }
            // error 发短信 warning 只发邮件
            foreach($logs as $log)  {
                if ($log[1] == "error" && $isSendSms) {
                    $sms = new SmsService();
                    $smsContent =  mb_substr($log[0], 0, 60);
                   $rs = $sms->send(Yii::app()->params['monitorMobile'], $smsContent);
    //var_dump(Yii::app()->params['monitorMobile'],$smsContent, $rs);
                }
            ...
            $this->sendYiiMail($subject, $message);
        } catch (Exception $e) {
            //echo $e->getMessage();
        }
	}

    protected function sendYiiMail($subject, $message) {
        $mail = new YiiMailMessage();
        $mail->setFrom(array('gaygrouperrorlogs@mazhan.com' => '交易报错'));
        $mail->setTo($this->getEmails());
        $mail->setSubject($subject);
        $mail->setBody($message);

        $sendmail = Yii::app()->mail->send($mail);
    }
