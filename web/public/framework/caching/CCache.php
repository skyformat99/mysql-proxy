...
abstract class CCache extends CApplicationComponent implements ICache, ArrayAccess
{
	 ...
	 */
	public function get($id,$auto_delete=true)
	{
		$env = getenv('RUNTIME_ENVIROMENT');
		if($env === 'TEST'&&$auto_delete===true){
			$this->delete($id);
		}
		if(isset($_GET['delete_cache']) && $_GET['delete_cache']=='remove')
		{
			$this->delete($id);
		}
		if(isset($_GET['delete_cache']) && $_GET['delete_cache']=='remove_all')
		{
			$this->flush();
		}
